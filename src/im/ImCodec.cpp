...
std::variant<im::Request,im::Response> im::tryParse(std::string_view payload){
    try{
        ...
        if(!j.contains("ver")||!j.contains("type")||!j.contains("req_id")){
            return im::Response{.ver=1,.req_id=0,.ok=false,.code=im::ErrorCode::MISSING_FIELD,.msg="Missing required field",.data=nlohmann::json{}};
        }
        uint32_t ver=j["ver"].get<uint32_t>();
        if(ver!=1){
            return im::Response{.ver=ver,.req_id=0,.ok=false,.code=im::ErrorCode::UNSUPPORTED_VER,.msg="Unsupported protocol version",.data=nlohmann::json{}};
        }
        uint32_t type_int=j["type"].get<uint32_t>();
        auto type_opt=im::msgTypeFromInt(type_int);
        if (!type_opt.has_value()){
            return im::Response{.ver=ver,.req_id=0,.ok=false,.code=im::ErrorCode::UNKNOWN_TYPE,.msg="Unknown message type",.data=nlohmann::json{}};
        }
        ...
        return im::Response{.ver=1,.req_id=0,.ok=false,.code=im::ErrorCode::BAD_JSON,.msg="Invalid JSON format",.data=nlohmann::json{}};
    }
}
