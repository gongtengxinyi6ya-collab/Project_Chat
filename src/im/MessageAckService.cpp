...
storage::RepoResult im::MessageAckService::ackOfflineMessages(const std::string&accountId,const std::vector<uint64_t>& offlineIds){
    if(accountId.empty()){
        return {.status=storage::RepoStatus::InvalidArgument,.message="accountId is empty"};
    }
    if(offlineIds.empty()){
    ...
    return offlineMessageRepo_->ackOfflineMessages(accountId,offlineIds);
}
