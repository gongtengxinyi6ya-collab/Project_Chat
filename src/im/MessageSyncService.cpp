...
storage::RepoValueResult<im::SyncResult> im::MessageSyncService::sync(const std::string& accountId,const std::vector<SyncCursor>& cursors,size_t offlineLimit){
    ...
    auto offlineResult=offlineMessageRepo_->listOfflineMessage(accountId,offlineLimit);
    if(!offlineResult.ok()||!offlineResult.value.has_value()){
        return {.status = offlineResult.status,.message = offlineResult.message};
    }
    syncResult.offlineIndexes=std::move(offlineResult.value.value());
    return {.status=storage::RepoStatus::Ok,.value=std::move(syncResult)};
}
