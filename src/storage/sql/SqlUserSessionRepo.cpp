...
std::optional<storage::StoredUserSession> storage::SqlUserSessionRepo::findByTokenHash(const std::string& tokenHash){
    ...
    if(conn->connected()){
        auto result=conn->queryPrepared("SELECT * FROM user_sessions WHERE token_hash=? LIMIT 1",{tokenHash});
...
storage::RepoResult storage::SqlUserSessionRepo::touchSession(const std::string& tokenHash,int64_t lastSeenAtMs){
    ...
    if(conn->connected()){
        auto result=conn->executePrepared("UPDATE user_sessions SET last_seen_at_ms=? WHERE token_hash=?",{lastSeenAtMs,tokenHash});
        if(result.ok()){
            return RepoResult{.status=RepoStatus::Ok};
        }
        return RepoResult{.status=RepoStatus::SqlError,.message=result.error};
...
storage::RepoResult storage::SqlUserSessionRepo::revokeSession(const std::string& tokenHash){
    if(tokenHash.empty()){
        return RepoResult{.status=RepoStatus::InvalidArgument};
    }
    auto conn=pool_->acquire();
    ...
    if(conn->connected()){
        auto result=conn->executePrepared("DELETE FROM user_sessions WHERE token_hash=?",{tokenHash});
        if(result.ok()){
            return RepoResult{.status=RepoStatus::Ok};
        }
        return RepoResult{.status=RepoStatus::SqlError,.message=result.error};
