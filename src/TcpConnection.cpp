...
void TcpConnection::handleWrite(){
    while(outputBuffer_.readableBytes()>0){
        ...
        if(n>0){//发送成功，清除outputBuffer_中字节
            ...
            size_t overload=overloadDropCount_.load(std::memory_order_relaxed);
...
bool TcpConnection::canSend(size_t payloadBytes)const{
    ...
    size_t pending=pendingBytesEstimate_.load(std::memory_order_relaxed);
    return connection_&&channel_!=nullptr&&pending+frameBytes<=hardLimit_;
}
...
void TcpConnection::recordDrop(size_t payloadBytes){
    ...
    if(drops>=maxOverloadDropCount_){
        ...
        LOG_WARN("Connection " + std::to_string(fd_) + " has dropped " +std::to_string(overloadDropCount_) + " messages due to overload, exceeding max overload drop count of " + std::to_string(maxOverloadDropCount_) + ", consider closing connection");
    }
}
