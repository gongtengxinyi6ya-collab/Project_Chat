...
void TcpConnection::handleWrite(){
    while(outputBuffer_.readableBytes()>0){
        ssize_t n=::send(fd_,outputBuffer_.peek(),outputBuffer_.readableBytes(),MSG_NOSIGNAL);//发送数据
        if(n>0){//发送成功，清除outputBuffer_中字节
            ...
            if(errno==EINTR){//继续本轮发送
                continue;
            }
            else if(errno==EAGAIN||errno==EWOULDBLOCK){//缓冲区满需等待下一次
                return;
            }
                ...
                LOG_ERROR("Failed to send msg,errno= "+ec.message());
                handleClose();
                return;
            }
        }
        ...
        if(outputBuffer_.readableBytes()==0){
            if(channel_&&!channel_->disableWriting()){
                LOG_ERROR("Failed to disableWritting");
                handleClose();
            }
        }   
...
void TcpConnection::handleError(int socketError){
    const std::error_code ec(socketError, std::system_category());
    LOG_ERROR("TcpConnection::handleError"+std::to_string(fd_)+", error: "+ec.message());
    handleClose();
}
