...
CREATE TABLE IF NOT EXISTS user_sessions (
    ...
    KEY idx_user_sessions_revoked_cleanup (revoked, revoked_at_ms)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS friend_requests (
    ...
    KEY idx_friend_requests_cleanup (
        status,
        handled_at_ms
    )
) ENGINE=InnoDB
...
CREATE TABLE IF NOT EXISTS group_members (
    ...
    role TINYINT NOT NULL DEFAULT 0
        COMMENT '0=member,1=admin,2=owner',
...
CREATE TABLE IF NOT EXISTS group_join_requests (
    ...
    KEY idx_group_join_requests_cleanup (
        status,
        reviewed_at_ms
    )
) ENGINE=InnoDB
...
-- =========================================================
-- 群消息表
-- group_seq 已直接合并到最终表结构
-- =========================================================
CREATE TABLE IF NOT EXISTS messages (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    msg_id BIGINT UNSIGNED NOT NULL,
    group_id VARCHAR(64) NOT NULL,
    group_seq BIGINT UNSIGNED NOT NULL,
    ...
    UNIQUE KEY uk_messages_group_seq (group_id, group_seq),
...
CREATE TABLE IF NOT EXISTS offline_messages (
    ...
    msg_type TINYINT UNSIGNED NOT NULL DEFAULT 1
        COMMENT '1=group,2=direct',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

    PRIMARY KEY (id),
    UNIQUE KEY uk_offline_user_msg (
        account_id,
        msg_id
    ),
    UNIQUE KEY uk_offline_account_type_msg (
        account_id,
        msg_type,
        msg_id
    ),
    KEY idx_offline_account_id_id (
        account_id,
        id
    ),
    KEY idx_offline_account_id_msg (
        account_id,
        msg_id
    ),
    KEY idx_offline_group_id (group_id),
    KEY idx_offline_peer_account_id (peer_account_id),
    KEY idx_offline_messages_cleanup (created_at)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS message_receipts (
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_msg_account (
        msg_id,
        account_id
    ),
    KEY idx_account_delivered (
        account_id,
        delivered_at_ms
    ),
    KEY idx_account_read (
        account_id,
        read_at_ms
    ),
...
-- 保存每个群当前最新序号及消息摘要
-- =========================================================
CREATE TABLE IF NOT EXISTS group_conversation_heads (
    group_id VARCHAR(64) NOT NULL,
...
-- 群成员已读游标表
-- =========================================================
CREATE TABLE IF NOT EXISTS user_group_cursors (
    account_id VARCHAR(32) NOT NULL,
    group_id VARCHAR(64) NOT NULL,
