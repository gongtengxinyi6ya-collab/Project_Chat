...
class GroupService{
public:
    GroupService(GroupManager& groupManager,snowflakeId::SnowflakeIdGenerator& idGenerator,std::shared_ptr<storage::GroupRepo> groupRepo,std::shared_ptr<storage::UserProfileRepo> userProfileRepo,std::shared_ptr<storage::FriendRepo> friendRepo,bool requireFriendForInvite,size_t maxGroupMembers);
    ...
    storage::RepoValueResult<GroupCreateResult> createGroup(const std::string& ownerAccountId,const std::string& groupName);//创建群聊
    ...
    storage::RepoValueResult<storage::GroupSearchResult> searchGroupById(const std::string&groupId,const std::string& requesterAccountId);//查找群聊
private:
