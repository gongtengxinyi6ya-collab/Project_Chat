...
public:
    ...
    storage::RepoResult acceptRequest(const std::string& accountId,uint64_t requestId,int64_t nowMs);//委托Repo执行事务化同意
    storage::RepoResult rejectRequest(const std::string&accountId,uint64_t requestId,int64_t nowMs);//委托Repo拒绝申请
