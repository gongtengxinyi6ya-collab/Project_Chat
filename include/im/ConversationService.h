...
public:
    ...
    ConversationService(std::shared_ptr<storage::ConversationRepo> conversationRepo,std::shared_ptr<storage::UserProfileRepo> userProfileRep);
    storage::RepoResult recordDirectMessage(const std::string&senderAccountId,const std::string&receiverAccountId,const std::string& senderUsername,uint64_t msgId,const std::string& content,uint64_t serverTsMs);//私聊消息保存成功后，更新双方会话列表
