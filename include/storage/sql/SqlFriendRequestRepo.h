...
public:
    ...
    RepoValueResult<FriendRequest> findById(uint64_t requestId)override;//查询指定申请
    RepoResult rejectPending(uint64_t requestId,const std::string& receiver,int64_t nowMs)override;//拒绝状态为待处理的申请
    RepoResult acceptPendingAndCreateFriendPair(uint64_t requestId,const std::string& receiver,int64_t nowMs)override;//事务内同意申请并建立好友关系
private:
