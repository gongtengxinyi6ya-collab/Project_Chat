...
public:
    ...
    RepoValueResult<std::vector<OfflineMessageIndex>> listOfflineMessage(const std::string& accountId,size_t limit)override;//查询某用户的离线消息索引
    RepoValueResult<size_t> ackOfflineMessagesBatch(const std::string& accountId,const std::vector<uint64_t>& msgIds)override;//客户端确认后删除离线消息索引
