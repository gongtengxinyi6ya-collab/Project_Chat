...
public:
    ...
    RepoValueResult<std::vector<MessageRecord>> listGroupMessages(const std::string&groupId,uint64_t beforeMsgId,size_t limit)override;//查询群历史消息，最多返回limit条
    ...
    RepoValueResult<std::vector<DirectMessageRecord>> listDirectMessages(const std::string& conversationKey,uint64_t beforeMsgId,size_t limit) override;//查询某个私聊会话的历史消息
    RepoValueResult<std::vector<DirectMessageRecord>> listDirectMessagesAfter(const std::string& conversationKey,uint64_t lastMsgId,size_t limit)override;//查询某个私聊会话客户端本地最后一条消息之后的消息
    RepoValueResult<std::vector<MessageRecord>> listGroupMessagesAfter(const std::string& groupId,uint64_t lastMsgId,size_t limit)override;//查询某个群聊中msg_id>lastMsgId的新消息
    RepoValueResult<MessageAckResult> markDeliveredBatch(const std::string&accountId,const std::vector<uint64_t>& msgIds,int64_t deliveredAtMs)override;//标记当前用户已经收到消息
