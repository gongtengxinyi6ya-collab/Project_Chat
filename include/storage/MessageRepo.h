...
struct DirectMessageRecord {
    ...
    virtual RepoResult markDelivered(const std::string&accountId,const std::vector<uint64_t>& msgIds,int64_t deliveredAtMs)=0;//标记当前用户已经收到消息
    virtual RepoResult markReadBefore(const std::string&accountId,ConversationType type,const std::string& targetId,uint64_t readMsgId,int64_t readAtMs)=0;//把某个会话中msg_id<=readMsgId的消息标记为当前用户已读
